...
class Controller extends \Gcms\Controller
{
    ...
    public static function addCard(Request $request, $card, $login)
    {
        ...
        \Index\Home\Controller::renderCard($card, 'icon-exchange', '{LNG_Borrow} &amp; {LNG_Return} ('.$login['name'].')', number_format($items->pending), Language::get('BORROW_STATUS', null, 0), 'index.php?module=borrow-setup&amp;status=0');
        \Index\Home\Controller::renderCard($card, 'icon-valid', '{LNG_Borrow} &amp; {LNG_Return} ('.$login['name'].')', number_format($items->confirmed), Language::get('BORROW_STATUS', null, 2), 'index.php?module=borrow-setup&amp;status=2');
