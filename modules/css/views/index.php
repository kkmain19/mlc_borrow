...
class View extends \Kotchasan\KBase
{
    ...
    public function index()
    {
        ...
        if (!empty(self::$cfg->login_color)) {
            $data2 .= '.welcomepage #login_div form{color:'.self::$cfg->login_color.'}';
        }
        ...
        if (!empty(self::$cfg->login_header_color)) {
            $data2 .= '.welcomepage #login_div h1,.welcomepage #login_div h3{color:'.self::$cfg->login_header_color.'}';
        }
        if (!empty(self::$cfg->warpper_bg_color)) {
            $data2 .= '.mainpage .wrapper{background-color:'.self::$cfg->warpper_bg_color.'}';
        }
