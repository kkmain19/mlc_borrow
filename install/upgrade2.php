    ...
    if (empty($_POST['username']) || empty($_POST['password'])) {
        ...
        if (!$error) {
            ...
            try {
                ...
                if ($db->fieldExists($table, 'visited')) {
                    $db->query("ALTER TABLE `$table` DROP `visited`");
                }
                if ($db->fieldExists($table, 'lastvisited')) {
                    $db->query("ALTER TABLE `$table` DROP `lastvisited`");
                }
                if ($db->fieldExists($table, 'session_id')) {
                    $db->query("ALTER TABLE `$table` DROP `session_id`");
                }
                if ($db->fieldExists($table, 'ip')) {
                    $db->query("ALTER TABLE `$table` DROP `ip`");
                }
